...
{
  public class UnapplyMetamorphVisualsSystem : ReactiveSystem<GameEntity> 
  {
    public UnapplyMetamorphVisualsSystem(GameContext game) : base(game)
    {
    }
    ...
    protected override void Execute(List<GameEntity> statuses)
    {
      foreach (GameEntity status in statuses)
      {
        ...
        if (target is {hasStatusVisuals: true}) 
          target.StatusVisuals.UnapplyMetamorph();
      }
    }
  }
}